...
apt update -y
apt full-upgrade -y
apt install python3-pip python-pip apt-transport-https wget curl clang git grep netcat net-tools screen apt-utils unzip pkg-config libfreetype6-dev python-bs4 tightvncserver php lxde-core metasploit-framework -y
# apt install virtualenv bettercap armitage aircrack-ng nmap sparta apktool xfe openjdk-13-jdk idle zipalign neofetch exiftool -y
apt purge udisks2 -y
